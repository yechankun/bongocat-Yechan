...
void draw() {
    ...
    window.draw(bg);
	static double ARM_LEFT_TOP = 208;

    // initializing pss and pss2 (kuvster's magic)
    int oof = 6;
    //std::vector<double> pss = {211.0, 159.0};
	std::vector<double> pss = { 211.0, ARM_LEFT_TOP };
    double dist = hypot(211 - x, ARM_LEFT_TOP - y);
    double centreleft0 = 211 - 0.7237 * dist / 2;
    double centreleft1 = ARM_LEFT_TOP + 0.69 * dist / 2;
    for (int i = 1; i < oof; i++) {
		//std::vector<double> bez = { 211, 159, centreleft0, centreleft1, x, y };
        std::vector<double> bez = {211, ARM_LEFT_TOP, centreleft0, centreleft1, x, y};
